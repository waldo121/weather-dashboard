...
class DomoticzClient:
    ...
    def send_weather_data_json(
        self,
        temperature: float,
        humidity: float,
    ) -> None:
        with httpx.Client(headers={"Authorization": f"Basic {self._authorization_token}"}) as client:
            response = client.get(f"{self._base_url}/json.htm?type=command&param=udevice&idx={self._weather_device_id}&nvalues=0&svalue={temperature};{humidity};0")
